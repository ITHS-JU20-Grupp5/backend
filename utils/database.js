...
const db = new sqlite.Database(DBFILE, (err) => {
  ...
  db.run(
    'CREATE TABLE category_questions(CategoryId integer not null, QuestionId integer not null, primary key(CategoryId, QuestionId), foreign key(CategoryId) references categories(Id), foreign key(QuestionId) references questions(Id))',
    cb
  );
  db.run(
    'CREATE TABLE question_answers(QuestionId integer not null, AnswerId integer not null, primary key(QuestionId, AnswerId), foreign key(QuestionId) references questions(Id), foreign key(AnswerId) references answers(Id))',
    cb
  );
  db.run(
  ...
  db.run(
    'CREATE TABLE user_scores(UserId integer not null, ScoreId integer not null, primary key(UserId, ScoreId), foreign key(UserId) references users(Id), foreign key(ScoreId) references scores(Id))',
    cb
  );
  db.run(
